...
public class ClientObjectWorker implements Runnable, ObserverInterface {
    ...
    public ClientObjectWorker(ServiceInterface server, Socket connection) {
        ...
        private Object handleRequest(Request request){
            ...
            if (request instanceof PlayersRequest) {
                ...
                try {
                    System.out.println("aici");
                    return new PlayersResponse((List<Child>) server.getAllPlayers());
                } catch (Exception e) {
                    connected = false;
                    return new ErrorResponse(e.getMessage());
                }
            ...
            if(request instanceof PointsRequest){
                ...
                Child player = pointsRequest.getPlayer();
                Integer point= pointsRequest.getPoint();
                String time = pointsRequest.getTime();
                try {
                    server.addPoint(player,time,point);
                    return new OkResponse();
                } catch (Exception e) {
                    return new ErrorResponse(e.getMessage());
                }
    ...
    public void addPoint(Child player, String time, Integer point) throws Exception {
        System.out.println("Points added "+player.getPoint());
        try {
            sendResponse(new PointsResponse(player,time,point));
        } catch (IOException e) {
